...
import { buildCatalog, normalizeGuaridaListing } from './lib/schema.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const root = path.resolve(__dirname, '..');
const configPath = path.join(root, 'config', 'filters.json');
const outputPath = path.join(root, 'data', 'guarida.json');

const config = JSON.parse(await fs.readFile(configPath, 'utf8'));
const sourceConfig = config.sources.guarida;

console.log(`Coletando Guarida — ${config.city}/${config.state}`);
console.log(`Filtro: valor total <= R$ ${config.maxTotalCost}`);

const result = await collectGuarida({
...
const listings = result.houses
  .map((house) => normalizeGuaridaListing(house))
