...
export class ExplorerPageComponent implements OnInit {
  ...
  readonly resultsSubtitle = computed(() => {
    ...
    const filtered = this.state.filtered();
    if (!filtered.length) return 'Ajuste bairros, valor, área, centralidade ou imobiliária.';
    ...
    const costHint = this.state.costRangeActive()
      ...
      : '';
    const areaHint = this.state.areaRangeActive()
      ? ` · ${this.state.minArea()}–${this.state.maxArea()} m²`
      : '';
    return `Média total ${formatBrl(avg)} · aderência média ${Math.round(avgFit)}% · ${high} com match total · ${this.presetLabel()}${costHint}${areaHint}`;
  });
